...
const addMenuItem = async ({name, cuisine, location, owner_id}) => {
    const result = {};
    result.status = false;
    result.message = '';
...
const getMenuList = async ({restaurantId}) => {
    const result = {};
    result.status = false;
    result.message = '';
    result.statusCode = 200;
    ...
    try {
        const menuList = await runQuery('select * from menus where is_deleted = ? and restaurant_id = ?', ["N",parseInt(restaurantId)]);
        if(menuList.status) {
            result.message = 'successfullly fetched the data'
            result.result = menuList.value
            result.status = true;
            return result;
        }
